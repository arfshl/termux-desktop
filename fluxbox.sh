...
cat <<'EOF' > /data/data/com.termux/files/usr/bin/startdesktop-fluxbox
#!/bin/sh
...
DISPLAY=:0 dbus-launch --exit-with-session fluxbox
EOF
...
export DISPLAY=:1
fluxbox-generate_menu
dbus-launch --exit-with-session fluxbox' >> ~/.vnc/xstartup
...
echo 'To start termux-x11 session use startdesktop-fluxbox command'

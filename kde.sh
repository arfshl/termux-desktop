...
# Install KDE Plasma
apt install -y tigervnc termux-x11 pulseaudio dbus plasma konsole dolphin

cat <<'EOF' > /data/data/com.termux/files/usr/bin/startdesktop-kde
#!/bin/sh
...
export XDG_RUNTIME_DIR=${TMPDIR}
DISPLAY=:0 dbus-launch --exit-with-session startplasma-x11
EOF
...
echo 'To start termux-x11 session use startdesktop-kde command'

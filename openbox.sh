...
cat <<'EOF' > /data/data/com.termux/files/usr/bin/startdesktop-openbox
#!/bin/sh
...
export PULSE_SERVER=127.0.0.1
export DISPLAY=:1
dbus-launch --exit-with-session openbox-session' >> ~/.vnc/xstartup
...
echo 'To start termux-x11 session use startdesktop-openbox command'
